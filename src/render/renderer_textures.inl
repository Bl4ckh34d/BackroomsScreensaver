...
// Included inside Renderer private section before flashlight-pattern texture creation.
    ...
    bool CreateLoosePageTextureArray() {
        ...
        for (int slot = 0; slot < pageCount; ++slot) {
            ...
            context_->UpdateSubresource(tex.Get(), subresource, nullptr, pixels.data(), pageW * 4, 0);
        }
        ...
        if (FAILED(device_->CreateShaderResourceView(tex.Get(), &sd, &loosePagesSrv_))) return false;
        context_->GenerateMips(loosePagesSrv_.Get());
        return true;
    }
