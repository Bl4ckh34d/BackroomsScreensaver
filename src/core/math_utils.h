...
inline float MoveTowards(float current, float target, float maxDelta) {
    ...
    if (std::abs(delta) <= maxDelta) return current + delta;
    return current + (delta > 0.0f ? maxDelta : -maxDelta);
}

inline float AngleWrap(float a) {
...
inline float Rand01(int x, int y, int z, uint32_t seed) {
...
inline float FractalNoise(float x, float y, uint32_t seed) {
    float sum = 0.0f;
    float amp = 0.55f;
    float freq = 1.0f;
    float norm = 0.0f;
