...
void RuntimeProfileGpuLine(const std::wstring& line) {
    ...
    if (!out) return;
    if (!wroteHeader) {
        out << L"frame,total_ms,clear_ms,dynamic_geometry_ms,flashlight_shadow_ms,fixture_shadow_ms,monster_eye_shadow_ms,uploads_ms,main_opaque_ms,floor_ceiling_ms,dynamic_opaque_ms,static_water_ms,static_transparent_ms,dynamic_transparent_ms,post_process_ms,overlays_ms\r\n";
        wroteHeader = true;
    }
    out << line << L"\r\n";
    if (++rowsSinceFlush >= 30) {
